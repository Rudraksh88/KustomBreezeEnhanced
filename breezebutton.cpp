...
namespace Breeze
{
    ...
    namespace
    {
        constexpr qreal MutedButtonOpacity = 0.2;
        ...
        QImage whiteTint(QImage image, qreal opacity, bool highlightDetails)
        {
            ...
            quint64 luminanceWeights[256] = {};
            for (int y = 0; y < image.height(); ++y) {
                const auto *pixels = reinterpret_cast<const QRgb *>(image.constScanLine(y));
                for (int x = 0; x < image.width(); ++x) {
                    const int alpha = qAlpha(pixels[x]);
                    if (alpha > 0)
                        luminanceWeights[qGray(pixels[x])] += alpha;
                }
            }

            int referenceLuminance = 255;
            quint64 greatestWeight = 0;
            for (int luminance = 0; luminance < 256; ++luminance) {
                if (luminanceWeights[luminance] > greatestWeight) {
                    greatestWeight = luminanceWeights[luminance];
            ...
            for (int y = 0; y < image.height(); ++y) {
                ...
                for (int x = 0; x < image.width(); ++x) {
                    ...
                    qreal contrast = 0.0;
                    if (luminance < referenceLuminance && referenceLuminance > 0) {
                        contrast = qreal(referenceLuminance - luminance) / referenceLuminance;
                    } else if (luminance > referenceLuminance && referenceLuminance < 255) {
                        contrast = qreal(luminance - referenceLuminance) / (255 - referenceLuminance);
                    }
                    ...
                    pixels[x] = qRgba(tint, tint, tint, qRound(qAlpha(pixel) * pixelOpacity));
                }
            }
            return image;
        }
    ...
    void Button::paintButton(QPainter *painter)
    {
        ...
        if (type() == DecorationButtonType::Menu)
        {
            ...
            if ( d && d->internalSettings()->buttonStyle() == 0 )
                ...
                drawIconMacMono( painter );
            else if ( d && d->internalSettings()->buttonStyle() == 15 )
                drawIconZephyr( painter );
        ...
        */
        ...
        switch( type() )
        {
            ...
            case DecorationButtonType::Maximize:
            {
                QColor button_color;
                if ( !inactiveWindow && qGray(titleBarColor.rgb()) < 128 )
                  button_color = QColor(255, 189, 68);
                else if( !inactiveWindow )
                  button_color = QColor(255, 189, 68);
                else if ( qGray(titleBarColor.rgb()) < 128 )
                  button_color = QColor(100, 100, 100);
    ...
    //__________________________________________________________________
    // Same as Mac Sierra but in monochrome with primary color asa #5A5A5A
    void Button::drawIconMacMono( QPainter *painter ) const {
        ...
        QColor symbolColor(QColor(255, 255, 255, 180));
        QColor outlineColor( QColor(255, 255, 255, 56) );
            ...
            (type() == DecorationButtonType::KeepBelow && isChecked()) )
        {
            ...
            switch( type() )
            {
                ...
                case DecorationButtonType::ApplicationMenu:
                {
                    QPen menuSymbol_pen( symbolColor );
                    menuSymbol_pen.setJoinStyle( Qt::MiterJoin );
    ...
    //__________________________________________________________________
    void Button::drawIconZephyr( QPainter *painter ) const {
      /*
      A simple button style that is used in the Zephyr decoration. - □ ✗

      For copilot:
      - These are simple Windows OS like buttons.
      - The buttons dont have a border.
      - The buttons only have a hover effect. Hover effect is a simple color change.
      - The buttons have a simple icon in the center.
      */
      ...
      */
      ...
      const qreal width( m_iconSize.width() + 0.5 ); // Value from the settings
      painter->scale( width/20, width/20 );
      painter->translate( 1, 1 );

      QColor darkSymbolColor = QColor(34, 45, 50);
      QColor lightSymbolColor = QColor(255, 255, 255, 125);
      QColor foregroundColor = QColor(255, 255, 255, 125);

      auto d = qobject_cast<Decoration*>( decoration() );
      // Change the pen color to white if animations are enabled
      QColor symbolColor = d->internalSettings()->animationsEnabled() ? QColor(255, 255, 255) : this->autoColor( false, true, false, darkSymbolColor, lightSymbolColor );

      // symbols pen
      const double STROKE_WIDTH = 1.6; // Stroke width of the icon
      QPen symbol_pen( foregroundColor );
      symbol_pen.setJoinStyle( Qt::MiterJoin );
      symbol_pen.setWidthF( STROKE_WIDTH*qMax((qreal)1.0, 20/width ) );

      switch( type() ) {
        case DecorationButtonType::Close: {
          // Draw the close button
          // painter->setPen( symbol_pen ); // Set the pen color
          // painter->drawLine( QPointF( 5, 5 ), QPointF( 13, 13 ) );
          // painter->drawLine( QPointF( 5, 13 ), QPointF( 13, 5 ) );

          QColor buttonColor = foregroundColor;
          qreal opacity = 1.0;
          ...
          if (isPressed()) {
              opacity = 1 - 0.3 * m_animation->currentValue().toReal();
          } else if (animationsEnabled && isHovered()) {
              opacity = 0.5 + 0.5 * m_animation->currentValue().toReal();
          } else if (!isHovered()) {
              opacity = 0.5 + 0.5 * m_animation->currentValue().toReal();
          }
          ...
          // Draw the close button
          painter->drawLine( QPointF( 5, 3.5 ), QPointF( 13, 11.5 ) );
          painter->drawLine( QPointF( 5, 11.5 ), QPointF( 13, 3.5 ) );
        ...
        case DecorationButtonType::Maximize: {
          ...
          // // Use rect to draw a square
          // painter->drawRect( QRectF(5, 5, 8, 8) );
          ...
          // Updated with current coordinates
          if (isChecked()) {
              painter->drawPolyline(QPolygonF() << QPointF(7.5, 2.5) << QPointF(14, 2.5) << QPointF(14, 8.5)); // -|
              painter->drawRect(QRectF(3.5, 5.5, 7.5, 7.0)); // x,y,w,h //
          } else {
        ...
        case DecorationButtonType::Minimize: {
          ...
          // painter->setPen( symbol_pen ); // Set the pen color
          // painter->drawLine( QPointF( 3, 10 ), QPointF( 12, 10 ) );
          ...
          // Draw the minimize button
          painter->drawLine( QPointF( 3, 8.5 ), QPointF( 12, 8.5 ) );
        ...
        case DecorationButtonType::OnAllDesktops: {
          ...
          QColor innerCircleColor = foregroundColor;
          QColor outerCircleColor = foregroundColor;
          qreal innerOpacity = 1.0;
          qreal outerOpacity = 0.0;
          qreal outerCircleSize = 18.0;  // Set to full size by default

          auto d = qobject_cast<Decoration*>(decoration());
          bool animationsEnabled = d && d->internalSettings()->animationsEnabled();

          if (isPressed() || isChecked()) {
              // innerOpacity = isChecked() ? 1.0 : 0.5;
              innerOpacity = isPressed() ? 0.5 : 1.0;
              outerOpacity = isChecked() ? 0.15 : 0.0;
              outerCircleSize = 18.0;
          } else if (animationsEnabled) {
              if (isHovered() && !isChecked()) {
                  innerOpacity = 0.5 + 0.5 * m_animation->currentValue().toReal();
              }
          ...
          innerCircleColor.setAlphaF(innerOpacity);
          outerCircleColor.setAlphaF(outerOpacity);
          ...
          // Always draw the outer circle, but with varying opacity
          painter->setPen(Qt::NoPen);
          painter->setBrush(outerCircleColor);
          painter->drawEllipse(QRectF(
              centerX - outerCircleSize / 2,
              centerY - outerCircleSize / 2,
              outerCircleSize,
              outerCircleSize
          ));
        ...
        case DecorationButtonType::KeepBelow: {
          ...
          // Draw two backslashes
          painter->drawLine(QPointF(5.5, 4), QPointF(10.5, 12));
          painter->drawLine(QPointF(10.5, 4), QPointF(15.5, 12));

          // Draw additional slash if checked
          if (isChecked()) {
              painter->drawLine(QPointF(0.5, 4), QPointF(5.5, 12));
          }
        ...
        case DecorationButtonType::KeepAbove: {
          ...
          // Draw two forward slashes
          painter->drawLine(QPointF(8, 11), QPointF(13, 4));
          painter->drawLine(QPointF(13, 11), QPointF(18, 4));

          // Draw additional slash if checked
          if (isChecked()) {
              painter->drawLine(QPointF(3, 11), QPointF(8, 4));
          }

          // // Draw two forward slashes
          // painter->drawLine(QPointF(5.5, 12), QPointF(10.5, 4));
          // painter->drawLine(QPointF(10.5, 12), QPointF(15.5, 4));

          // // Draw additional slash if checked
          // if (isChecked()) {
          //     painter->drawLine(QPointF(0.5, 12), QPointF(5.5, 4));
          // }
